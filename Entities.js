...
class MotionDetector {
  constructor (name, initialIntensity) {
    ...
    this.name = name || 'unnamed detector.'
...
class BaseNotifier {
  constructor (name) {
    this.name = name || 'Default Base Notifier'
    events.EventEmitter.call(this)
    this.detectors = []
    this.internalObj
  }
 ...
 */
  ...
  get_declarative_pattern (prop) {
    if (prop.startsWith(reservedPatterns.NEW)) { return reservedPatterns.NEW }
    if (prop == reservedPatterns.DETECTORS) { return reservedPatterns.DETECTORS }
  }
...
function GetExtensionsMetadata (instanceName, appendObj) {
  let result = GetExtensions(instanceName)
  ChangeMetadataExtensions(result, appendObj)
  return result
}

// Appends metadata to the instance names belonging inside the extensions
function ChangeMetadataExtensions (extensions, appendObj) { //, instanceName, handler){
  for (let i in extensions) {
    ...
    if (classRequired.prototype._meta) {
      ...
      // Create a new key
      extensions[i + ': ' + classRequired.prototype._meta.description()] = handler(classRequired, appendObj)
